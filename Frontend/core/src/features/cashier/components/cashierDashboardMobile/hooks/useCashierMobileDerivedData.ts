...
export const useCashierMobileDerivedData = ({
  ...
  const statsForCard = useMemo(() => ({
    ...
    cashPayments: allOrders.filter((order) => order.payment_method === 'efectivo').length,
    transferPayments: allOrders.filter((order) => order.payment_method === 'transferencia').length,
