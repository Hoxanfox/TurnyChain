...
export const useCashierMobileDerivedData = ({
  ...
  const statsForCard = useMemo(() => ({
    ...
    cashPayments: allOrders.filter((order) => order.payments?.some(p => p.method === 'efectivo') || order.payment_method === 'efectivo').length,
    transferPayments: allOrders.filter((order) => order.payments?.some(p => p.method === 'transferencia') || order.payment_method === 'transferencia').length,
